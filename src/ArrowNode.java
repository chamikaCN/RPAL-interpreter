public class ArrowNode extends _MultipolarNode implements Standardizable {
    ...
    public void standardize() {
        ...
        GammaNode gamma1 = new GammaNode(par, level);
        par.removeAddChild(gamma1, thisIndex);

        GammaNode gamma2 = new GammaNode(gamma1, level + 1);
        gamma1.addChild(gamma2, 0);

        LEAF_NilNode nil = new LEAF_NilNode(gamma1, level + 1);
        gamma1.addChild(nil, 1);

        GammaNode gamma3 = new GammaNode(gamma2, level + 2);
        gamma2.addChild(gamma3, 0);

        LambdaNode lambda1 = new LambdaNode(gamma2, level + 2);
        gamma2.addChild(lambda1, 1);

        LEAF_ParenthesisNode paranthesisF = new LEAF_ParenthesisNode(lambda1, level + 3);
        lambda1.addChild(paranthesisF, 0);
        lambda1.addChild(F, 1);
        F.setParent(lambda1);

        GammaNode gamma4 = new GammaNode(gamma3, level + 3);
        gamma3.addChild(gamma4, 0);

        LambdaNode lambda2 = new LambdaNode(gamma3, level + 3);
        gamma3.addChild(lambda2, 1);

        LEAF_IDnode cond = new LEAF_IDnode(gamma4, "Cond", level + 4);
        gamma4.addChild(cond, 0);
        gamma4.addChild(B, 1);
        B.setParent(gamma4);

        LEAF_ParenthesisNode paranthesisT = new LEAF_ParenthesisNode(lambda2, level + 4);
